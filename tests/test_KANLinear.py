...
#     input_tensor = jnp.ones((batch_size, in_features))  # batch size of 3, in_features of 10
...
#     # Check output shape
#     expected_output_shape = (batch_size, out_features)  # batch size of 3, out_features of 5
...
def test_update_grid_and_spline_weights(torch_module, flax_module):
    ...
    for name, param in torch_module.named_buffers():
        numpy_param = param.detach().numpy()
        flax_params = unfreeze(flax_params)
        flax_params['buffers'][name] = numpy_param
        flax_params = freeze(flax_params)
    ...
    _, flax_params = flax_model.apply(flax_params,
                                      ...
                                      compare = jnp.array(unreduced_spline_output_t.detach().numpy()),
...
#     # Compute splines with the correct grid argument
#     splines_f = flax_model.b_splines(input_tensor_f, old_params['grid'])  # (batch, in_features, coeff)
...
#     A_f = flax_model.b_splines(input_tensor_f, grid_f).transpose((1, 0, 2))  # (in_features, batch_size, grid_size + spline_order)
...
#     np.testing.assert_allclose(result_t.detach().numpy(), np.array(result_f), atol=1e-4, rtol=1e-4, err_msg="result do not match")